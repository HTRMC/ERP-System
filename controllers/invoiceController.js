  ...
  try {
    ...
    clockEntries.forEach((entry) => {
      ...
      const hours = entry.clock_out_time
        ? (new Date(entry.clock_out_time) - new Date(entry.clock_in_time)) /
          (1000 * 60 * 60)
        : 0;
      if (!userHours[userId]) {
        userHours[userId] = 0;
    ...
    const totalHours = Object.values(userHours).reduce(
    ...
    const now = new Date();
    const invoiceId = `${now.getFullYear()}${String(
      now.getMonth() + 1
    ...
    doc
      .fontSize(10)
      .text(`Invoice Number: INV-${invoiceId}`, 200, 65, { align: "right" });
    doc
      .fontSize(10)
      .text(`Invoice Date: ${now.toISOString().split("T")[0]}`, 200, 80, {
        align: "right",
      });
    doc
      .fontSize(10)
    ...
    doc
      .fontSize(12)
      .text("Tax (10%): €" + (totalCost * 0.1).toFixed(2), 400, position + 15);
    doc
      .fontSize(12)
      .text("Total: €" + (totalCost * 1.1).toFixed(2), 400, position + 30);
    doc.moveDown();
